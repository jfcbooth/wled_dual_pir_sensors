...
private:
  ...
  int8_t PIRsensorPin       = 5; // PIR sensor pin
  int8_t PIRsensorPin2      = 18; // PIR sensor pin
  uint32_t m_switchOffDelay = 120;         // delay before switch off after the sensor state goes LOW (10min)
  ...
  {
    bool pinState = digitalRead(PIRsensorPin) || digitalRead(PIRsensorPin2);
...
public:
  ...
  void setup()
  {
    if (enabled) {
      ...
      if ((PIRsensorPin >= 0 && pinManager.allocatePin(PIRsensorPin, false, PinOwner::UM_PIR)) && (PIRsensorPin2 >= 0 && pinManager.allocatePin(PIRsensorPin2, false, PinOwner::UM_PIR))) {
        // PIR Sensor mode INPUT_PULLUP
        pinMode(PIRsensorPin, INPUT_PULLUP);
        pinMode(PIRsensorPin2, INPUT_PULLUP);
        sensorPinState = digitalRead(PIRsensorPin) || digitalRead(PIRsensorPin2);
      } else {
        if (PIRsensorPin >= 0 && PIRsensorPin2 >= 0) {
          DEBUG_PRINTLN(F("PIRSensorSwitch pin allocation failed."));
        }
        PIRsensorPin = -1;  // allocation failed
        PIRsensorPin2 = -1;  // allocation failed
        enabled = false;
      }
    }
    initDone = true;
